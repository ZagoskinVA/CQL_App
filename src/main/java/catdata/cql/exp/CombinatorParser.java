...
import com.example.program.InstExpStream;
...
public class CombinatorParser implements IAqlParser {
	...
	private static void instExp() {
		...
		Parser ret = Parsers.or(queryQuotientExpRaw(), sigma_chase, l2, pi, frozen, instExpRand(), instExpCoEq(),
				...
				except, pivot, cod, skolem, instExpCsv(), instExpStream(), coeval, /* excel, */ instExpJdbcDirect(), instExpTinkerpop(), parens(inst_ref));
	...
	private static void transExp() {
		...
		Parser ret = Parsers.or(id, id1, transExpRaw(), var, sigma, skolem, delta, unit, counit, distinct, eval, coeval,
				transExpCsv(), transExpStream(), pi, distinct_return, frozen, except2, unitq, except, counitq, transExpJdbc(), comp,
...
//---------------------------------------------------------------------------
	private static Parser<InstExpStream> instExpStream() {
		Parser<catdata.Pair<List<catdata.Pair<LocStr, String>>, List<catdata.Pair<String, String>>>> b = Parsers
				.tuple(token("{"), env(ident, "->"), options, token("}")).map(x -> new catdata.Pair<>(x.b, x.c));
		...
		Parser<InstExpStream> ret = Parsers.tuple(token("import_stream"), ident.followedBy(token(":")), sch_ref.lazy(), qs)
				.map(x -> new InstExpStream(x.c, x.d.a, x.d.b, x.b));
		return ret;
	}

	@SuppressWarnings({ "rawtypes", "unchecked" })
	private static Parser<TransExpStream> transExpStream() {
		Parser<Pair<List<catdata.Pair<LocStr, String>>, List<catdata.Pair<String, String>>>> qs = Parsers
				.tuple(env(ident, "->"), options).between(token("{"), token("}"));

		Parser<TransExpStream> ret = Parsers.tuple(token("import_stream").followedBy(token(":")),
				inst_ref.lazy().followedBy(token("->")), inst_ref.lazy(), qs)
				.map(x -> new TransExpStream(x.b, x.c, x.d.a, x.d.b));
		return ret;
	}
